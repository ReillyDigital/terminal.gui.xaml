...
namespace ReillyDigital.Terminal.Gui.Xaml.Generation.Sources
{
	...
	public class ClassSource
	{
		...
		private string SourceClassWrapper(params string[] contents) => $@"
			public partial class {ClassName} : INotifyPropertyChanged, INotifyPropertyChanging, ITerminalXamlView
			{{
				{string.Join("\r\n", contents)}
			}}
		";
		...
		private string SourceDataContextProperty()
		{
			...
			return $@"
				...
				{{
					...
					set
					{{
						PropertyChanging?.Invoke(this, new PropertyChangingEventArgs(nameof(DataContext)));
						DataContextChanging?.Invoke(this, new EventArgs());
						_DataContext = value;
		...
		private string SourceInitializeComponentMethod()
		{
			...
			foreach (var attribute in Xaml.Root.Attributes().Where((attribute) => !attribute.IsNamespaceDeclaration))
			{
				...
				if (bindingParts.Success)
				{
					...
					{
						returnValue += $@"
							EventHandler {attributeName}Changed = (object? sender, EventArgs e) =>
							{{
								...
								{attributeName} = DataContext.{bindingParts.Groups[1].Value};
							}};
							DataContextChanged += {attributeName}Changed;
							if (DataContext is not null)
							{{
								{attributeName}Changed(this, new EventArgs());
							}}
						";
					}
					continue;
				...
				if (bindingParts.Success)
				{
					if (!(ModelClassName is null))
					{
						returnValue += $@"
							...
							DataContextChanging += (object? sender, EventArgs e) =>
							{{
								if (DataContext is null)
								{{
									return;
		...
		/// A <see cref="string" /> of the generated source code.
		/// </returns>
		private string SourceInitializeComponentMethodElement(
			XElement element, int index, string parentVariableName = null, string direction = "Vertical"
		)
		{
			var returnValue = "";
			...
			{
				returnValue += $@"
					var {variableName} = new View();
					{variableName}.Height = Dim.Fill();
					{variableName}.Width = Dim.Fill();
				";
			...
			foreach (var attribute in element.Attributes().Where((attribute) => !attribute.IsNamespaceDeclaration))
			{
				...
				if (bindingParts.Success)
				{
					...
					{
						returnValue += $@"
							EventHandler {variableName}_{attributeName}Changed = (object? sender, EventArgs e) =>
							{{
								if (DataContext is null)
								{{
									{variableName}.{attributeName} = default!;
									return;
								}}
								{variableName}.{attributeName} = DataContext.{bindingParts.Groups[1].Value};
							}};
							DataContextChanged += {variableName}_{attributeName}Changed;
							if (DataContext is not null)
							{{
								{variableName}_{attributeName}Changed(this, new EventArgs());
							}}
						";
					}
					continue;
